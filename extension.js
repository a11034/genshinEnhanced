...
game.import("extension", function (lib, game, ui, get, ai, _status) {
	return {
		...
		precontent: function () {
			...
			lib.init.js(`${lib.assetURL}extension/原梗Enhanced/lib`, 'cnchar.min');
    ...
    `;
			...
			get.playAnimation = function (videoPath) {
				return new Promise(resolve => {
					game.broadcastAll(() => {
						const oldVideo = document.querySelector(".yge-anim");
						if (oldVideo) oldVideo.remove();

						const video = document.createElement("video");
						video.className = "yge-anim";
						Object.assign(video, {
							src: videoPath,
							autoplay: true,
						});
			...
			// 集中管理所有势力配置（可扩展、易维护）-武将势力
			const groups = [
				{ id: 'lvzhe', abbr: '旅', name: '旅者', color: '#fff176' },
				{ id: 'mengde', abbr: '蒙', name: '蒙德', color: '#43a047', image: "ext:原梗Enhanced/image/mark/mengde.png" },
				{ id: 'liyue', abbr: '璃', name: '璃月', color: '#b71c1c', image: "ext:原梗Enhanced/image/mark/liyue.png" },
				{ id: 'daoqi', abbr: '稻', name: '稻妻', color: '#ab47bc', image: "ext:原梗Enhanced/image/mark/daoqi.png" },
				{ id: 'xumi', abbr: '须', name: '须弥', color: '#66bb6a', image: "ext:原梗Enhanced/image/mark/xumi.png" },
				{ id: 'zhidong', abbr: '至', name: '至冬', color: '#546e7a' },
				{ id: 'seer', abbr: '赛', name: '赛尔', color: '#5ED5FD', image: "ext:原梗Enhanced/image/mark/seer.png" },
			];

			// 使用 for...of 与解构一次性注册-数据与注册分离
			for (const { id, abbr, name, ...options } of groups) {
				game.addGroup(id, abbr, name, options);
			}
			...
			game.import('character', function (lib, game, ui, get, ai, _status) {
				var genshinImact = {
					...
					character: {//这里写武将
						...
						xiaoA: ["male", "liyue", 3, ["nuomianA", "yezhangA", "xiangmoA"], []],
						...
						babalaA: ["female", "mengde", 3, ["shanyaoA", "qihuA", "lianyinA", "chunxinA"], []],
						banniteA: ["male", "mengde", 4, ["chiyaoA", "xianluA", "huixingA"], []],
						xiaogongA: ["female", "daoqi", 4, ["yanxiaoA", "xiangzhuA"], []],
						shenlilingrenA: ["male", "daoqi", 4, ["quanmouA", "jinghuaA", "fenchuanA"], []],
						...
						bachongshenziA: ["female", "daoqi", 4, ["yizhouA", "xianzhenA", "shenleA"], []],
						shanhugongxinhaiA: ["female", "daoqi", "1/5/3", ["miaosuanA", "hairanA", "yuehuaA"], []],
						wulangA: ["male", "daoqi", 4, ["cixiongA", "fujiA", "zhixinA"], []],
						huanglongyidouA: ["male", "daoqi", 5, ["haoyongA", "guiwangA", "langgeA"], []],
						...
						Seer_gaiya: ["male", "seer", 4, ["seer_zhanhun", "seer_jingtian", "seer_kuangwu"], []]
					},
					characterSort: {//武将分包
						GenshinImpactEnhanced: {
							...
							qiyuezhiyan: ['hutaoA', 'zhongliA', 'ganyuA', 'qiqiA', 'keqingA', 'ningguangA', 'shenheA', 'yaoyaoA', 'yelanA', 'yunjinA', 'xianglingA', 'xingqiuA', 'xiaoA', 'lanyanA'],
							...
							yiciyuan: ["puniA", "Seer_leiyi", "Seer_gaiya"],
					...
					characterIntro: {//武将的介绍
						...
						Seer_gaiya: `<div style="padding:1rem;text-align:center;display:flex;align-items:center;justify-content:center;gap:0.5rem;"><span style="color:#FF0000;font-size:1.5rem;font-weight:bold;font-style:italic;letter-spacing:1px;">即使全世界都站在你对立面呢？“力求无愧于心！”</span></div>`
					},
					characterTitle: {
						puniA: "赛尔信仰·神灵",
						Seer_leiyi: "怒雷神裁",
					...
					characterSubstitute: {
						...
						youlaA: [["优菈-使命成功", ["ext:原梗Enhanced/other/优菈使命达成.png"]], ["优菈-使命失败", ["ext:原梗Enhanced/other/优菈使命失败.png"]]]
					},
					...
					skill: {
						HunYinA: {
							...
							subSkill: {
								...
								"3": {
									...
									async content(event, trigger, player) {
										const result = await player.chooseTarget([1, Infinity])
											...
											.set("ai", target => -get.attitude(target, get.player()))
											.forResult();
										if (result.bool) {
											for (const target of result.targets) {
												await target.damage(trigger.num * 2, "notrigger", "nosource");
									...
									ai: {
										...
										effect: {
											target: function (card, player, target) {
												if (get.tag(card, "damage")) return [1, 0, 1, -10];
											},
										},
									},
									sub: true,
								...
								"4": {
									forced: true,
									trigger: {
										player: ["changeHpBefore", "loseMaxHpBefore"],
									},
									filter: function (event, player, name) {
										if (_status.currentPhase === player) return false;
										return name === "changeHpBefore" ? event.num < 0 : event.num > 0;
									},
									async content(event, trigger, player) {
										trigger.cancel();
									},
						...
						HunYinB: {
							init: (player, skill) => player.addMark(skill, 3),
						...
						seer_zhanhun: {
							...
							filter: function (event, player, name) {
								return player.isPhaseUsing() && !player.getStorage("seer_zhanhun").includes(get.suit(event.card));
							},
							getAllDamageCards() {
								return lib.inpile.flatMap(name => {
									if (get.type(name) === 'delay' || !get.tag?.({ name }, 'damage')) return [];
									const baseCards = [game.createCard({ name, suit: lib.suit.randomGet(), number: get.rand(1, 13) })];
									if (name === 'sha') {
										const natureCards = lib.inpile_nature.map(nature =>
											game.createCard({ name, nature, suit: lib.suit.randomGet(), number: get.rand(1, 13) })
										);
										return [...baseCards, ...natureCards];
									}
									return baseCards;
								});
							},
							async content(event, trigger, player) {
								...
								await player.gain(get.info(event.name).getAllDamageCards().filter(card => !player.hasCard(cardx => get.name(cardx) === get.name(card) &&
									(get.name(card) !== 'sha' || get.nature(cardx) === get.nature(card)), 'h')), "gain2");
							},
							group: "seer_zhanhun_buff",
							subSkill: {
								buff: {
									audio: "seer_zhanhun",
									forced: true,
									...
									async content(event, trigger, player) {
										player.unmarkSkill("seer_zhanhun");
										player.removeTip("seer_zhanhun");
										await player.discard(player.getDiscardableCards(player, "j"));
									},
									mod: {
										cardUsable: function (card, player) {
							...
							mod: {
								aiOrder: function (player, card, num) {
									...
									参考技能【渐营】【奋音】【联翩】*/
									if (player.getStorage("seer_zhanhun").includes(get.suit(card))) {
										if (player.getStorage("seer_zhanhun").length >= 4 && card.name == "sha") return num + 20;
										return num * 2;
									}
								},
							},
							ai: {
								effect: {
									player_use: function (card, player, target, current) {
										if (!player.getStorage("seer_zhanhun").includes(get.suit(card))) return [1, 10];
									},
								},
							},
						},
						seer_jingtian: {
							...
							filter: function (event, player, name) {
								if (player.getStorage("seer_zhanhun").length < 4) return false;
								return event.targets.length == 1 && event.target != player && event.card && get.name(event.card, player) == "sha";
							},
							async content(event, trigger, player) {
								...
								map[id].extraDamage += trigger.target.maxHp;
								trigger.getParent().directHit.addArray(game.filterPlayer());
								trigger.target.removeSkills(trigger.target.getStockSkills(false, true, true).filter(skill => !get.info(skill).charlotte));
							},
							ai: {
								directHit_ai: true,
								skillTagFilter(player, tag, arg) {
									return player.getStorage("seer_zhanhun").length >= 4;
								},
							},
						},
						seer_kuangwu: {
							...
							filter: function (event, player, name) {
								return event.source && event.source != event.player && !event.player.getHistory("sourceDamage", (evt) => evt.player == event.source).length;
							},
							async content(event, trigger, player) {
								trigger.cancel();
							},
							group: "seer_kuangwu_reset",
							subSkill: {
								reset: {
									audio: "seer_kuangwu",
									forced: true,
									...
									async content(event, trigger, player) {
										player.restoreSkill("seer_jingtian");
									},
									sub: true,
								},
							},
						...
						xuehuoA: {
								...
								directHit: {
									...
									"targetprompt2": target => {
										...
										if (player.hp <= Math.ceil(player.maxHp / 2)) {
											list.add(`<span style="color:#fff;text-shadow:0 0 6px #00BFFF,0 0 12px #00BFFF,0 0 2px #0000FF;font-weight:bold">不可响应</span>`);
										}
										return list;
									},
									onChooseToUse(event) {
										event.targetprompt2.add(lib.skill.xuehuoA_directHit.targetprompt2);
									},
							...
							mod: {
								...
								aiValue: function (player, card, num) {
									...
									参考技能【立牧】【新服缮甲】*/
									if (card.name == "sha") {
										// 手里还能打出多少张杀（只算没被限制且目标合法的）
										let num1 = player.countCards("h", card => card.name == "sha" && player.hasUseTarget(card, true, true));
										// 本回合最多还能使用几张杀（考虑技能、装备、次数限制）
										let num2 = player.getCardUsable("sha", true)
										// 缺杀：手里能打的张数 < 本回合还可打的上限 → 价值翻倍
										if (num1 < num2) {
											return num * Math.max(player.getDamagedHp(), 2)
										}
									}
									if (get.tag(card, 'fireDamage')) return num * Math.max(player.getDamagedHp(), 2);
								},
								aiOrder: function (player, card, num) {
									/*更改AI使用卡的顺序(player为你，card为要判断的牌，num为上次判断后的order值)
									参考技能【渐营】【奋音】【联翩】*/
									if (get.tag(card, 'recover')) return Math.floor(num / player.hp);
								},
							},
							ai: {
								unequip: true,
						...
						wangshengA: {
							...
							async content(event, trigger, player) {
								...
								let gainCards = trigger.player.getCards('he');
								if (gainCards.length) await player.gain(gainCards, trigger.player, "draw2");
								const list = trigger.player.getStockSkills(true, true, true);
						...
						yuzhangA: {
							...
							mod: {
								...
								/* card是要判断的牌，player是获得事件的来源，target才是你，name是事件名称，now是现在的值 */
								canBeGained: function (card, player, target, name, now) {
									if (target.hasMark("yuzhangA") && ['h', 'e'].includes(get.position(card)) && player != target)
										return false;
								},
						...
						fengshenA: {
							...
							async content(event, trigger, player) {
								...
								for (const target of event.targets) {
									await target.damage(Math.max(discardNumber, 3), "chenshaA");
								}
							},
							check: function (card) {
						...
						jichouA: {
							...
							onremove: function (player, skill) {
								let cards = player.getExpansions("jichouA");
								if (cards.length) player.loseToDiscardpile(cards);
							},
							trigger: {
								player: "damageEnd",
							...
							filter: function (event, player) {
								...
								return event.source.hasCards("he");
							},
							async content(event, trigger, player) {
								const next = player.addToExpansion(trigger.source, trigger.source.getCards("he"), 'give');
								next.gaintag.add(event.name);
								await next;
							...
							subSkill: {
								1: {
									audio: "ext:原梗Enhanced/audio/skill/jichouA_1.mp3",
									unique: true,
									forced: true,
									trigger: {
										player: "phaseBegin",
									...
									filter: function (event, player) {
										return event.player.hasExpansions("jichouA");
									},
									async content(event, trigger, player) {
										const cards = player.getExpansions("jichouA");
										await player.loseToDiscardpile(cards);
										const cardx = get.autoViewAs({ name: "sha", nature: "ice", isCard: true });
										for (let i = 0; i < cards.length; i++) {
											const result = await player.chooseUseTarget(cardx, "nodistance")
												.set("prompt", `记仇：请选择冰【杀】的目标`)
												.set("addCount", false)
												.forResult();
											if (!result?.bool) {
												break;
						...
						zhuomengA: {
							audio: "ext:原梗Enhanced/audio/skill:2",
							frequent: true,
							init: (player) => player.storage.zhuoMengCards = ["chaojuanbingxueA", "guangchaohuanxiangA", "ninglangguangjianA"],
							trigger: {
								player: "useCardAfter",
							},
							filter: function (event, player) {
								return get.name(event.card, player) == "jiu";
							},
							async content(event, trigger, player) {
								const gainCards = [];
								for (const card of player.storage.zhuoMengCards) {
									gainCards.add(game.createCard(card));
								}
								await player.gain(gainCards, 'draw2');
							},
							group: ["zhuomengA_1"],
							subSkill: {
								1: {
									...
									filter: function (event, player) {
										return event.player != player && !player.hasHistory('damage') && player.inRangeOf(event.player);
									},
									async content(event, trigger, player) {
						...
						jiawuA: {
							...
							filter: function (event, player) {
								return event.player != player && player.hasMark("jiawuA_mark");
							},
							async content(event, trigger, player) {
								player.removeMark('jiawuA_mark', 1, false);
								game.log(player, "移除了一层枷锁");
							},
							group: ["jiawuA_mark", "jiawuA_achieve", "jiawuA_fail"],
							subSkill: {
								mark: {
									silent: true,
									unique: true,
									...
									async content(event, trigger, player) {
										player.addMark(event.name, 3);
									},
									mod: {
										cardEnabled2: function (card, player) {
											if (["shan", "tao"].includes(card.name)) return false;
										},
								...
								achieve: {
									audio: "ext:原梗Enhanced/audio/skill/jiawuA_achieve.mp3",
									unique: true,
									forced: true,
									skillAnimation: true,
									animationStr: "使命达成",
									...
									filter: function (event, player) {
										return !player.hasMark("jiawuA_mark");
									},
									async content(event, trigger, player) {
										player.awakenSkill('jiawuA');
										player.changeSkin({ characterName: "youlaA" }, "优菈-使命成功");
										player.popup('使命达成');
										player.addSkill('langwuA');
									},
								},
								fail: {
									audio: "ext:原梗Enhanced/audio/skill/jiawuA_fail.mp3",
									unique: true,
									forced: true,
									trigger: {
										player: "dying",
									...
									async content(event, trigger, player) {
										player.awakenSkill('jiawuA');
										player.changeSkin({ characterName: "youlaA" }, "优菈-使命失败");
										player.popup('使命失败');
							...
							mod: {
								aiValue: function (player, card, num) {
									/*更改AI对牌回合内使用价值的判断(player为你，card为要改变value的卡，num为上次判断后的value值)*/
									if (["shan", "tao"].includes(card.name)) return 0;
									if (["nanman", "wanjian"].includes(card.name)) return num * 3;
								},
							...
							ai: {
								effect: {
									player: function (card, player, target) {
										if (["nanman", "wanjian"].includes(card.name)) return [1, 10];
									},
								},
							},
							"_priority": 0,
						...
						langwuA: {
							...
							filter: function (event, player) {
								return !player.getStorage("langwuA_used").includes(event.card.name);
							},
							async content(event, trigger, player) {
								...
								const gainCardList = [];
								for (let i = 0; i < 3; i++) {
									let card = get.cardPile2(cardx => get.type(cardx) === "trick" && !gainCardList.includes(cardx));
									if (card) gainCardList.push(card);
									else break;
								}
								if (gainCardList.length) await player.gain(gainCardList, "gain2")
							},
							...
							subSkill: {
								damage: {
									...
									async content(event, trigger, player) {
										const damageNumber = player.getHistory("useCard").length;
										const result = await player.chooseTarget()
											.set("prompt", `###浪舞###对一名其他角色造成${get.cnNumber(damageNumber)}点冰冻伤害`)
											.set("filterTarget", (card, player, target) => player != target)
											.set("ai", function (target) {
											...
											.forResult();
										if (result.bool) {
											await result.targets[0].damage(damageNumber, "ice");
										}
									},
									sub: true,
								},
							...
							ai: {
								effect: {
									player_use: function (card, player, target) {
										if (!player.getStorage("langwuA_used").includes(card.name)) return [1, 5];
									},
								},
							},
							"_priority": 0,
						...
						dueA: {
							...
							async content(event, trigger, player) {
								if (["trick", "basic"].includes(get.type2(trigger.player.judging[0]))) {
									const damageResult = await player.chooseTarget()
										.set("filterTarget", (card, player, target) => target != player)
										.set("prompt", "###渡厄###对一名其他角色造成1点冰冻伤害")
										.set("ai", (target) => get.damageEffect(target, player, player, "ice")).forResult();
									if (damageResult.bool) damageResult.targets[0].damage(1, "ice");
									player.logSkill(event.name);
								}
								if (player.countCards("hes") > 0) {
									const result1 = await player.chooseCard("hes")
										.set("prompt2", `${get.translation(trigger.player)}的${trigger.judgestr || ""}判定为${get.translation(trigger.player.judging[0])}，${get.prompt(event.name)}`)
										.set("filterCard", card => {
											const player = get.player();
										...
										.set("ai", card => {
											...
											if (attitude == 0 || result == 0) {
												return get.value(judging, player) - get.value(card, player);
											}
											if (attitude > 0) {
												return result - val;
											}
									...
									if (result1.bool) {
										const { cards } = await player.respond(result1.cards, event.name, "highlight", "noOrdering");
										if (cards?.length) {
											player.logSkill(event.name);
						...
						yanmingA: {
							...
							async content(event, trigger, player) {
								const result = await trigger.player.judge((card) => (get.suit(card) == 'spade' && get.number(card) > 1 && get.number(card) < 10) ? -1 : 1).forResult();
								if (result.bool) {
									await trigger.player.recoverTo(1);
									await trigger.player.draw(trigger.player.maxHp);
								}
								else
									game.trySkillAudio("yanmingB", player, true, false, { audio: "ext:原梗Enhanced/audio/skill:2" });
							},
							ai: {
								threaten: 0.8,
							},
						...
						huihaiA: {
							...
							animationColor: "water",
							selectTarget: 1,
							filterTarget: (card, player, target) => target.isDead(),
							selectCard: -1,
							filterCard: () => false,
							filter: function (event, player) {
							...
							async content(event, trigger, player) {
								await get.playAnimation(lib.assetURL + "extension/原梗Enhanced/other/huihaiA1.mp4");
								const target = event.targets[0];
								const MaxHp = get.character(target.name).maxHp;//考虑失去上限死亡的角色，将其恢复成武将牌上的体力上限
								target.maxHp = MaxHp;
								await player.loseMaxHp();
								await target.reviveEvent(MaxHp);
								await target.drawTo(MaxHp);
							},
							ai: {
								order: 7,
								threaten: 2,
						...
						huohuaA: {
							...
							filter: function (event, player) {
								...
								return typeof number == "number" && number > 0 && get.type(event.card) == "basic";
							},
							isPrimeNumber(num) {
							...
							async content(event, trigger, player) {
								...
								const cardColor = get.color(trigger.card, trigger.player);
								if (get.info(event.name).isPrimeNumber(cardNumber)) {
									const result1 = await trigger.player.chooseTarget([1, Infinity])
										.set("prompt", "###火花###请选择要交给你牌的其他角色")
										.set("filterTarget", (card, player, target) => target != player)
									...
									if (result1.bool) {
										for (let target of result1.targets.sortBySeat()) {
											...
											const next = await target.chooseToGive(player)
												.set("filterCard", (card, player) => cardColor == get.color(card, player))
												.set("prompt", '交给' + get.translation(player) + '一张颜色为' + get.translation(cardColor) + '的牌，否则你受到1点无来源的火焰伤害')
												.set("ai", function (card) {
													let self = get.player();
													let dmg = get.damageEffect(self, self, self, "fire"); // 不交牌的伤害期望
													let myValue = get.value(card, self);                  // 牌在自己手里的价值
													let oppValue = get.value(card, player);               // 牌在对方手里的价值
													// 如果伤害很大（负值表示对自己不利），则更倾向于交牌
													if (dmg < 0) {
														// 综合考虑：伤害代价 vs 交牌代价
														// 交牌代价 = 自己失去的价值 + 对方获得的价值
														let giveCost = myValue + oppValue;
														// 如果伤害代价比交牌代价更大 → 返回正值（交牌）
														return -dmg - giveCost;
													}
													// 如果伤害对自己没影响甚至有利 → 不交牌
													return 0;
												...
												.forResult();
											if (!next?.bool) await target.damage(1, "fire", "nosource");
										}
									}
									trigger.player.addMark("huohuaA", 2);
								}
								else {
									const result = await player.chooseUseTarget("请选择火杀的目标", "nodistance")
										.set("card", { name: "sha", nature: "fire", storage: { huohuaA: true } })
										.set("selectTarget", [1, Infinity])
										.set("addCount", false)
										.forResult();
									if (!result.bool) player.chat("琴团长，可莉不要被关禁闭")
								}
							},
							mod: {
								aiOrder: function (player, card, num) {
									/*更改AI使用卡的顺序(player为你，card为要判断的牌，num为上次判断后的order值)
									参考技能【渐营】【奋音】【联翩】*/
									if (get.info("huohuaA").isPrimeNumber(card.number) && get.type(card) == "basic") return num + 2;
								},
								aiValue: function (player, card, num) {
									/*更改AI对牌回合内使用价值的判断(player为你，card为要改变value的卡，num为上次判断后的value值)
									参考技能【立牧】【新服缮甲】*/
									let canUse = game.hasPlayer(current => player.canUse(card, current, true, true));
									if (get.info("huohuaA").isPrimeNumber(card.number) && canUse && get.type(card) == "basic") return num + 3;
								},
							},
							group: "huohuaA_damage",
							subSkill: {
								damage: {
									audio: "huohuaA",
									unique: true,
									...
									filter: function (event, player) {
										return event.card && event.card.name == "sha" && event.card.storage.huohuaA && event.player.isIn();
									},
									async content(event, trigger, player) {
										const num1 = trigger.player.countCards("h", card => get.tag(card, "damage"));
										const num2 = trigger.player.countCards("h", card => !get.tag(card, "damage"));
										if (num1 > num2) trigger.player.addTempSkill("fenjinA", { player: "phaseAfter" });
										else trigger.player.addTempSkill("mbzhixi", { player: "phaseAfter" });
									},
									sub: true,
								},
							},
						...
						fenjinA: {
							...
							filter: function (event, player) {
								...
								return event.cards.filterInD("od").length > 0 && event.player.isIn();
							},
							async content(event, trigger, player) {
								let number = Math.max(trigger.player.getDamagedHp(), 1);
								if (get.color(trigger.card, trigger.player) == "red")
									await trigger.player.damage(number, "fire");
								else {
									player.addToExpansion(trigger.cards.filterInD("od"), "gain2").gaintag.add('fenjinA');
									game.log(player, "将" + get.translation(trigger.cards.filterInD("od")) + "置于了武将牌上");
								}
							},
							mod: {
								"cardEnabled2": function (card, player) {
									const expansions = player.getExpansions("fenjinA");
									if (expansions.some(card1 => card1.name == card.name)) {
										return false;
									}
								},
							},
							ai: {
								effect: {
									player_use: function (card, player, target) {
										if (get.color(card, player) == "red") return [1, -2 * player.getDamagedHp()];
										else return [1, -3];
									},
								},
							},
						...
						baodanA: {
							...
							animationColor: "fire",
							selectTarget: 1,
							filterTarget: (card, player, target) => player != target && target.hp <= player.countMark("huohuaA"),
							filter: function (event, player) {
								return game.hasPlayer(current => current.hp <= player.countMark("huohuaA"));;
							},
							async content(event, trigger, player) {
								let num = player.countMark("huohuaA");
								player.clearMark("huohuaA");
								await event.target.damage(num, "fire");
								if (event.target.isAlive()) event.target.removeSkills(event.target.getStockSkills(false, true, true).randomGet());
							},
							ai: {
								order: 10,
								threaten: 3,
								result: {
									target: (player, target) => -2 * player.countMark("huohuaA") - 10,
								},
							},
						},
						tianjuanA: {
							...
							async content(event, trigger, player) {
								const cards = await trigger.player.draw(2).forResult();
								const types = [];
								for (let card of cards) {
									types.add(get.type(card));
								}
								if (types.includes("basic")) trigger.cancel();
							},
							group: "tianjuanA_draw",
							subSkill: {
								draw: {
									audio: "tianjuanA",
									frequent: true,
									...
									async content(event, trigger, player) {
										...
										let cardList = [];
										for (let i of primes) {
											cardList.add(get.cardPile(card => get.number(card, false) == i))
										}
										if (cardList.length) await player.gain(cardList, "draw");
									},
									sub: true,
								},
							},
						...
						leifaA: {
							...
							content: function () {
								"step 0"
								player
									.discardPlayerCard(trigger.target, get.prompt("leifaA", trigger.target), 'he', true, "visible")
									.set("ai", function (button) {
						...
						shanyaoA: {
							...
							async content(event, trigger, player) {
								...
								if (trigger.name === 'gameDraw') {
									...
									trigger.num = (player2) => player2 == me ? 10 : typeof numx == "function" ? numx(player2) : numx;
								}
								trigger.gaintag ??= {};
								trigger.gaintag[me.playerid] = function (num, cards) {
									return [[cards, 'shanyaoA_tag']];
								};
							},
							mod: {
								ignoredHandcard(card, player) {
									if (card.hasGaintag("shanyaoA_tag")) {
										return true;
									}
								},
								cardDiscardable(card, player, name) {
									if (name == "phaseDiscard" && card.hasGaintag("shanyaoA_tag")) {
										return false;
									}
								},
							},
							...
							subSkill: {
								draw: {
									...
									filter: function (event, player, name) {
										...
										const ids = cards.filter(card => get.type2(card) === 'basic' && card.name != "du").map(card => card.cardid);
										if (!ids.length) return false;

										if (event.name === "lose") {
											const map = event.gaintag_map;
											return Object.keys(map).some(id => ids.includes(id) && map[id].includes("shanyaoA_tag"));
										}
									...
									async content(event, trigger, player) {
										const cards = trigger.getl(player).hs.filter(card => get.type2(card) === 'basic' && card.name != "du");
										const maps = [];
										if (trigger.name === "lose") {
											maps.push(trigger.gaintag_map);
										} else {
											player.getHistory("lose", evt => {
												if (evt.getParent() !== trigger) {
													return false;
												}
												maps.push(evt.gaintag_map);
											});
										}
										const num = cards.filter(card =>
											maps.some(map => map[card.cardid] && map[card.cardid].includes("shanyaoA_tag"))
										).length * 2;
										const result = await player.draw(num).forResult();
										if (result.cards?.length) player.addGaintag(result.cards, "shanyaoA_tag");
									},
									ai: {
										effect: {
											player_use: function (card, player, target, current) {
												if (get.type2(card) == "basic" && card.hasGaintag("shanyaoA_tag")) return [1, 2];
											},
										},
									},
									sub: true,
						...
						qihuA: {
							...
							filter: function (event, player) {
								return player.hasCards("h", card => card.hasGaintag("shanyaoA_tag") && lib.filter.canBeDiscarded(card, player, player));
							},
							async cost(event, trigger, player) {
								let target = trigger.player;
								const result = await player
									.chooseToDiscard(get.prompt("qihuA", trigger.player), [1, 3])
									.set("filterCard", card => card.hasGaintag("shanyaoA_tag"))
									.set("ai", function (card) {
										...
										if (att < 1) return 0;
										// 2. 身份局特殊限制：内奸不以这种方式帮主公
										if (get.mode() == "identity" && player.identity == "nei" && target.identity == "zhu") return 0;
										// 3. 如果目标是自己：统一返回 6 - get.value(card)
										if (target === player) {
											return 6 - get.value(card);
										}
										// 4. 目标是队友时：
										const hasJudge = target.hasCards("j", ["lebu", "bingliang", "shandian"]);
										const shaCount = target.mayHaveSha(player, "use", null, "count");
										const selectedCount = ui.selected.cards.length;
										// 选择第一张牌的逻辑：队友有判定牌 OR 队友有杀
										if (selectedCount === 0) {
											if (hasJudge || shaCount > 0) {
												return 6 - get.value(card);
											}
										}
										// 选择第二张、第三张牌的逻辑：仅取决于队友是否有杀
										else if (selectedCount === 1 || selectedCount === 2) {
											if (shaCount > 0) {
												return 6 - get.value(card);
											}
										}
							...
							async content(event, trigger, player) {
								const cardNumber = event.cards.length;
								if (cardNumber >= 1) await trigger.player.discard(trigger.player.getDiscardableCards(player, "j"));
								if (cardNumber >= 2) {
									trigger.player.addTempSkill("qihuA_dist");
									if (cardNumber >= 3) trigger.player.addTempSkill("qihuA_attack");
									trigger.player.addTempSkill("qihuA_mark");
								}
							},
							subSkill: {
								mark: {
									...
									intro: {
										...
										content: function (storage, player, skill) {
											const effects = [
												`<span style="display:block;text-align:center;font-weight:bold;color:#FFE655;text-shadow:0 0 2px #FFFFFF, 0 0 5px #FF9900;">元气迸发</span>使用基本牌无次数距离限制`,
												`<span style="display:block;text-align:center;font-weight:bold;color:#FF77DD;text-shadow:0 0 2px #FFFFFF, 0 0 5px #FF007F;">努力即魔法</span>使用基本牌的牌面数值+1`
											];
											if (player.hasSkill("qihuA_attack")) return effects.join("<br>");
											if (player.hasSkill("qihuA_dist")) return effects[0];
										},
									},
									sub: true,
								},
								...
								attack: {
									...
									filter(event, player) {
										return event.card && get.type2(event.card) === "basic";
									},
									async content(event, trigger, player) {
										trigger.baseDamage++;
									},
						...
						lianyinA: {
							...
							selectCard: [1, 3],
							position: "h",
							filterCard: card => card.hasGaintag("shanyaoA_tag"),
							...
							check(card) {
								const player = get.player();
								if (game.countPlayer((target) => target != player && get.effect(target, 'lianyinA', player, player) > 0) <= ui.selected.cards.length) return 0;
								return 6 - get.value(card);
							},
							filter: function (event, player) {
								return player.hasCards("h", card => card.hasGaintag("shanyaoA_tag") && lib.filter.canBeDiscarded(card, player, player));
							...
							async content(event, trigger, player) {
								const list = [[0, `令${get.translation(player)}获得你手牌中的所有【杀】并标记为“星”`], [1, "翻面"]];
								for (const target of event.targets) {
									await target.recoverTo(target.maxHp);
									const result = await target.chooseButton(["涟音：请选择一项：", [list, "textbutton"]])
										.set("forced", true)
										.set("selectButton", 1)
										.set("filterButton", button => button.link == 0 ? get.player().hasCards("h", card => card.name == "sha") : true)
										.set("ai", function (button) {
											const target = get.player();
											if (button.link === 1 && target.isTurnedOver()) return 1;
											if (target.hasCard(card => card.name == "sha", "h") && !target.isTurnedOver() && button.link === 0) return 1;
											return Math.random();
										}).forResult();
									if (result.bool) {
										if (result.links[0] === 0) {
											let next = player.gain(target.getCards("h", card => card.name === "sha"), "gain2")
											next.gaintag.add("shanyaoA_tag");
											await next;
										}
										else
							...
							ai: {
								...
								result: {
									target: function (player, target) {
										const damageHp = target.getDamagedHp();       // 损失的体力值
										const isTurnedOver = target.isTurnedOver();   // 是否已翻面
										// 计算回复体力的收益（适当调小基础值，采用平滑的二次方增长）
										let hpValue = 0;
										if (damageHp > 0) {
											hpValue = damageHp * damageHp;
										}
										let effectValue = 0;
										if (isTurnedOver) {
											effectValue = 4;
						...
						chunxinA: {
							...
							async content(event, trigger, player) {
								trigger.cancel();
								const nameList = ["sha", "shan", "tao", "jiu"];
								const gainCardList = nameList.map(name => game.createCard(name, lib.suit.randomGet(), get.rand(1, 13)));
								let next = player.gain(gainCardList, "gain2");
								next.gaintag.add("shanyaoA_tag");
								await next;
							},
							group: "chunxinA_save",
							subSkill: {
								save: {
									...
									enable: "chooseToUse",
									filter: function (event, player) {
										return game.hasPlayer(player => player.isDying());
									},
									viewAs: {
										name: "tao",
									},
									viewAsFilter: function (player) {
										return player.hasCards("h", card => card.hasGaintag("shanyaoA_tag"));
									},
									filterCard: card => card.hasGaintag("shanyaoA_tag"),
									position: "h",
									prompt: "将一张“星”当做【桃】使用",
									check: function (card) {
							...
							ai: {
								effect: {
									target: function (card, player, target, current) {
										...
										if (card.name === "damage" && get.event().card?.name === "sha") return;
										if (get.is.damageCard(card) && get.type(card) != "basic") return [0, 5];
									},
								},
							},
						},
						...
						yizhouA: {
							...
							intro: {
								name: "樱",
								mark: function (dialog, content, player) {
									var content = player.getExpansions('yizhouA');
							...
							filter: function (event, player, name) {
								if (!get.tag(event.card, "damage")) return false;
								if (player.getExpansions("yizhouA").length >= 20) return false;
								return event.target != player && event.target.countGainableCards(event.player, "he") > 0;
							},
							async content(event, trigger, player) {
								let number = player.getExpansions("yizhouA").length;
								const result = await player.choosePlayerCard(trigger.target, [1, 3], 'he', false, "visible")
									.set("prompt", "请选择" + get.translation(trigger.target) + "的至多三张牌")
									.set('ai', (button) => player.maxHp <= number ? 0 : get.buttonValue(button))
									.forResult();
								if (result.bool) {
									player.addToExpansion(result.links, trigger.target, 'give').gaintag.add('yizhouA');
								}
							...
							subSkill: {
								1: {
									...
									async content(event, trigger, player) {
										player.addToExpansion(get.cards(7), 'draw').gaintag.add('yizhouA');
									},
									sub: true,
								},
							},
							ai: {
								effect: {
									/* player函数里a、b、c、d,a、b是对你的收益，c、d是对你使用牌的目标的收益、
											 ...
											 */
									player: function (card, player, target) {
										if (player != target && get.tag(card, "damage")) return [1, 3];
									},
								},
							},
							"_priority": 0,
						...
						yizhouB: {
							...
							async content(event, trigger, player) {
								let list = player.getExpansions("yizhouA");
								const suitsCount = new Set(list.map(card => card.suit)).size;
								let gainNumber = 2 * suitsCount;
								const result = await trigger.player.chooseToDiscard(gainNumber, 'he', "请弃置" + get.cnNumber(gainNumber) + "张牌")
									.set("prompt2", "否则此牌对" + get.translation(player) + "无效且其获得此牌对应的实体牌。")
									.forResult();
								if (!result.bool) {
									trigger.getParent().excluded.add(player);
							...
							ai: {
								effect: {
									...
									target: function (card, player, target) {
										if (player != target) {
											let list = target.getExpansions("yizhouA");
											const suitsCount = new Set(list.map(card => card.suit)).size;
											let gainNumber = 2 * suitsCount;
											if (player.countCards("he") < gainNumber) return "zerotarget";
											return [1, 0, 1, -gainNumber];
										}
									},
								},
						...
						xianzhenA: {
							audio: "ext:原梗Enhanced/audio/skill:2",
							locked: true,
							usable: (skill, player) => player.maxHp,
							unique: true,
							enable: ["chooseToUse", "chooseToRespond"],
							filter(event, player) {
								if (!player.getExpansions('yizhouA').length > 0) return false;
							...
							chooseButton: {
								dialog(event, player) {
									...
									const dialog = ui.create.dialog('请选择一张基本牌', [list, 'vcard'], '请选择一张‘樱’', player.getExpansions('yizhouA'));
									return dialog;
								},
								/**
								...
								backup: function (links, player) {
									var next = {
										...
										precontent() {
											...
											player.addTempSkill('xianzhenA_1');
										},
									};
									return next;
								},
							...
							hiddenCard: function (player, name) {
								...
								return name != "du" && get.type(name) == "basic" && player.hasExpansions("yizhouA");
							},
							mod: {
								targetInRange: function (card, player, target, now) {
							...
							ai: {
								...
								fireAttack: true,
								skillTagFilter: function (player, tag, arg) {
									return player.hasExpansions('yizhouA');
								},
								result: {
							...
							onremove(player, skill) {
								var cards = player.getExpansions("yizhouA");
								if (cards.length) player.loseToDiscardpile(cards);
							},
						},
						shenleA: {
							...
							async content(event, trigger, player) {
								...
								for (const target of event.targets) {
									let number = [1, 2].randomGet();
									if (number == 1) await target.loseMaxHp(9);
									else {
										for (let i = 0; i < 9; i++) {
							...
							subSkill: {
								1: {
									...
									filter: function (event, player) {
										...
										return event.reason?.getParent().name == "shenleA";
									},
									async content(event, trigger, player) {
										await player.gainMaxHp();
										let gainCardNameList = lib.inpile.filter(c => get.type(c) == "trick");
										let gainCardList = [];
										for (var i = 0; i < 3; i++) {
						...
						miaosuanA: {
							...
							content: function () {
								...
								'step 2'
								let maxNumber = Math.max(game.countPlayer(), player.hp);
						...
						hairanA: {
							...
							async content(event, trigger, player) {
								const result = await player.chooseTarget()
									.set("filterTarget", (card, player, target) => target != player)
									.set("prompt", get.prompt2("hairanA"))
									.set("ai", (target) => -get.attitude(player, target)).forResult();
								if (result.bool) {
									const target = result.targets[0];
									player.logSkill("hairanA", target);
						...
						nuomianA: {
							...
							filter: function (event, player) {
								return event.player != player && event.player.hasCard((card) => !card.hasGaintag("nuomianA_tag"), "h");
							},
							async content(event, trigger, player) {
								const result = await player.choosePlayerCard(trigger.player, 1, "h", "visible")
									.set("filterButton", (button) => !button.link.hasGaintag("nuomianA_tag"))
									.set("forced", true)
									.set("prompt", "请选择要“裂解”的一张牌")
									.set("ai", function (button) {
										const target = trigger.player;
										const card = button.link;
										const splitTypes = new Set(
											target.getCards('h', c => c.hasGaintag('nuomianA_tag'))
												.map(c => get.type(c, 'trick'))
										);
										const cardType = get.type(card, 'trick');
										let score = get.value(card);      // 基础价值
										if (!splitTypes.has(cardType)) {
											score += 20;                  // 如果是新类型，加额外分数
											if (cardType == "basic") score += 5;//基本牌再加点分数
										}
										return score;
									})
									.forResult();
								if (result.bool) {
									result.links[0].addGaintag("nuomianA_tag");
								}
							},
							mod: {
								targetEnabled: function (card, player, target) {
									if (player != target && get.name(card, player) == "sha" && target.countCards("hs") <= player.countCards("hs"))
										return false;
								},
							},
							global: "nuomianA_debuff",
							subSkill: {
								debuff: {
									silent: true,
									charlotte: true,
									superCharlotte: true,
									onremove(player) {
										player.removeGaintag("nuomianA_tag");
									},
									trigger: {
										player: "loseAfter",
										global: ["equipAfter", "addJudgeAfter", "gainAfter", "loseAsyncAfter", "addToExpansionAfter"],
									...
									filter: function (event, player) {
										...
										if (event.name == "lose") {
											for (var i in event.gaintag_map) {
												if (event.gaintag_map[i].includes("nuomianA_tag")) {
													return true;
												}
											}
											return false;
										...
										return player.hasHistory("lose", function (evt) {
											...
											for (var i in evt.gaintag_map) {
												if (evt.gaintag_map[i].includes("nuomianA_tag")) {
													return true;
												}
											}
											return false;
									...
									mod: {
										cardEnabled2: function (card, player) {
											const splitTypes = new Set(
												player.getCards('h', c => c.hasGaintag('nuomianA_tag'))
													.map(c => get.type(c, 'trick'))
											);
											if (splitTypes.has(get.type(card, "trick"))) {
												return false;
											}
										},
									},
							...
							check: function (event, player) {
								return player.countCards("h", card => {
									return game.hasPlayer(target => {
										...
										return player.canUse(cardx, target, false, false) && get.effect(target, cardx, player, player) > 0;
									});
								});
							},
							prompt2: "令你的所有手牌于此阶段内均视为风【杀】",
							filter: function (event, player) {
								return player.countCards("h");
							},
							async content(event, trigger, player) {
								player.addTempSkill("yezhangA_attack", { player: 'phaseUseAfter' });
								player.addTempSkills(["yezhangA_buff", "yezhangA_debuff"], { player: 'phaseAfter' });
							},
							subSkill: {
								attack: {
									charlotte: true,
								...
								debuff: {
									...
									filter: function (event, player) {
										if (!event.card || event.card.name !== 'sha' || event.player === player || event.player.isAlive()) return false;
									...
									ai: {
										...
										effect: {
											/*
											 1.player函数里a、b、c、d,a、b是对你的收益，c、d是对你使用牌的目标的收益、
											 2.target函数里a、b、c、d，a、b仍是对你的收益（此时你是其他人用牌的目标），c、d是对牌的使用者的收益（对你用牌的那个角色）
											 */
											player: function (card, player, target) {
												if (card.name == "sha" && player != target) {
													if (ui.selected.targets.length + 1 >= player.hp / 2) return [1, -20];
												}
											},
										},
									},
						...
						xiangmoA: {
							audio: "ext:原梗Enhanced/audio/skill:2",
							derivation: ["jingyaoA", "lianpo", "rewansha"],
							charlotte: true,
							forced: true,
							juexingji: true,
							skillAnimation: true,           // 开启技能动画
							animationStr: "降妖伏魔",      // 动画文字
							animationColor: "fire",        // 动画颜色(fire/thunder/water/metal/soil)
							trigger: {
								global: "phaseBegin",
							},
							filter(event, player) {
								return player.hp <= 2 || player.countCards("h") <= 2;
							},
							async content(event, trigger, player) {
								player.awakenSkill(event.name);
								await player.recoverTo(player.maxHp);
								player.removeSkill("yezhangA");
								player.addSkills(["jingyaoA", "lianpo", "rewansha"]);
							},
						},
						jingyaoA: {
							audio: "ext:原梗Enhanced/audio/skill:2",
							...
							subSkill: {
								attack: {
									...
									"targetprompt2": target => {
										const card = get.card();
										if (card?.name !== 'sha' || !target.classList.contains('selectable')) return [];
										return [
											`<span style="color:#90EE90;text-shadow:0 0 6px #00BFFF,0 0 2px #008B8B;font-weight:bold">不可响应</span>`
										];
									},
									onChooseToUse(event) {
										event.targetprompt2.add(lib.skill.jingyaoA_attack.targetprompt2);
									},
									onChooseTarget(event) {
										event.targetprompt2.add(lib.skill.jingyaoA_attack.targetprompt2);
									},
									...
									filter: function (event, player) {
										return event.card && event.card.name == 'sha' && game.hasPlayer((current) => current != player && event.targets.includes(current));
									},
									async content(event, trigger, player) {
										let targets = game.filterPlayer((current) => current != player && trigger.targets.includes(current));
										trigger.directHit.addArray(targets);
						...
						haoyongA: {
							...
							async content(event, trigger, player) {
								let juedou = new lib.element.VCard({ name: 'juedou', storage: { haoyongA: true } });
								const result1 = await player.chooseControl("赤鬼之血", "赤牛发破")
									.set("displayIndex", false)
									.set("prompt", get.translation(player) + "发动了【豪勇】，请选择一项")
									.set('choiceList', [
										"失去1点体力，然后本回合使用牌无次数距离限制",
										"视为使用一张可指定任意目标的且不可被无懈的【决斗】",
									])
									.set('ai', () => {
										const player = get.player(), juedou = new lib.element.VCard({ name: 'juedou', storage: { haoyongA: true } });
										if (!player.hasValueTarget(juedou) || player.isZhu) return 0;
										return [0, 1].randomGet();
									}).forResult();
								if (result1.index === 1) {
									const next = player.chooseUseTarget(juedou, [1, Infinity]);
									const result2 = await next.forResult(), useCardEvent = player.getHistory('useCard', evt => evt.getParent() === next)[0];
									if (useCardEvent && result2?.bool && result2.targets?.length) {
										let loseTargetList = [];
										for (const target of result2.targets) {
											const juedouEvent = game.getGlobalHistory('everything', evt => {
												if (evt.name !== 'juedou') return false;
												return evt.target === target && evt.getParent() === useCardEvent;
											})[0];
											if (juedouEvent && juedouEvent.turn === player) continue;//juedou的上级事件是useCard，useCard的上级事件的chooseUseTarget，所以用历史往下寻找juedou事件即可，如果经历了决斗事件且turn属性在你这里就是你输了，否则就是经历了决斗事件且turn属性在对方那边或者这个目标没走决斗事件，就算你赢了
											loseTargetList.add(target);
										}
										if (loseTargetList.length >= Math.ceil(result2.targets.length / 2)) {
											for (const target of loseTargetList) {
												target.storage.haoyongA_defeated = player;
												target.addTempSkill("haoyongA_defeated", "roundStart");
											}
										}
									}
								}
									...
									await player.loseHp()
									player.addTempSkill('haoyongA_dist');
								}
							},
							mod: {
								wuxieRespondable(card, player) {
									if (card.storage?.haoyongA) return false;
								},
							},
							group: "haoyongA_defend",
							subSkill: {
								defend: {
									audio: "ext:原梗Enhanced/audio/skill:2",
									usable: 3,
									forced: true,
									trigger: {
										player: ["changeHpBefore", "gainMaxHpBefore", "loseMaxHpBefore"]
									},
									filter(event, player) {
									...
									ai: {
										...
										effect: {
											target: function (card, player, target, current) {
												if (_status.currentPhase !== target && (get.tag(card, 'damage') || get.tag(card, 'recover') || get.tag(card, 'loseHp'))) return 0;
											},
										},
									},
								},
								...
								defeated: {
									...
									intro: {
										content: function (storage, player) {
											return "本轮游戏使用牌只能指定" + get.translation(storage) + "为目标";
										}
									},
									mod: {
										playerEnabled: function (card, player, target) {
											if (player.storage.haoyongA_defeated && player.storage.haoyongA_defeated != target) {
												return false;
											}
										},
									},
							...
							ai: {
								effect: {
									player: function (card, player, target) {
										if (card.name == "juedou" && player != target && card.storage.haoyongA) {
											if (ui.selected.targets.length + 1 >= player.mayHaveSha(player, "respond", null, "count")) return [1, -20];
										}
									},
								},
							},
						...
						guiwangA: {
							...
							group: ["guiwangA_1", "guiwangA_3", "guiwangB"],
							subSkill: {
								1: {
									silent: true,
									trigger: {
								...
								2: {
									...
									"targetprompt2": target => {
										...
										if (card?.name !== 'sha' || !target.classList.contains('selectable')) return [];
										return [
											`<span style="color:#D2B48C; font-weight:900; text-shadow: 2px 2px 3px #5C4033, 0 0 8px rgba(139,69,19,0.6); letter-spacing: 2px;">不可响应</span>`
										];
									},
									onChooseToUse(event) {
										event.targetprompt2.add(lib.skill.guiwangA_2.targetprompt2);
									},
									onChooseTarget(event) {
										event.targetprompt2.add(lib.skill.guiwangA_2.targetprompt2);
									},
						...
						guiwangB: {
							...
							async content(event, trigger, player) {
								player.awakenSkill(event.name);
								trigger.cancel()
								const next = trigger.player.insertPhase();
						...
						langgeA: {
							...
							filter(event, player) {
								const lastEvt = player.getAllHistory("useCard", evt => evt != event).at(-1);
								if (!lastEvt?.card || !event.card || !get.tag(event.card, "damage")) return false;
								const sameName = get.name(lastEvt.card) === get.name(event.card);
								const sameSuit = get.suit(lastEvt.card) === get.suit(event.card);
								const sameNumber = get.number(lastEvt.card) === get.number(event.card);
								return !(sameName && sameSuit && sameNumber);
							},
							async cost(event, trigger, player) {
								const choiceList = ["获得一张【无中生有】和一张【推心置腹】", "获得一张【酒】和一张【兄弟齐心】", "获得一张【杀】和一张【决斗】"];
								const choices = ['选项一', '选项二', "选项三", 'cancel2'];
								const result = await player
									.chooseControl()
									.set('controls', choices)
									.set('choiceList', choiceList)
									.set('prompt', get.translation(player) + "发动了【朗歌】，请选择一项")
									.set("ai", () => {
										const player = get.player();
										const groups = [["wuzhong", "tuixinzhifu"], ["jiu", "qixin"], ["sha", "juedou"]];// 三组牌名
										const values = groups.map(group => group.reduce((sum, cardName) => sum + player.getUseValue(cardName), 0));  // 计算每组的总价值 
										const bestIndex = values.indexOf(Math.max(...values)); // 找出最大值对应的索引
										return [0, 1, 2].includes(bestIndex) ? bestIndex : [0, 1, 2].randomGet();//防御性编程
									})
									.forResult();
								event.result = {
							...
							async content(event, trigger, player) {
								...
								// 所有选项的牌名组合
								const cardOptions = {
									"选项一": ["wuzhong", "tuixinzhifu"],
									"选项二": ["jiu", "qixin"],
									"选项三": ["sha", "juedou"]
								};
								// 根据 event.cost_data 获取牌名数组
								const chosenCards = cardOptions[event.cost_data]
									.map(name => game.createCard(name, lib.suit.randomGet(), numberList.randomGet()));
								// 给玩家 
								await player.gain(chosenCards);
							},
						},
						leilunA: {
							audio: "ext:原梗Enhanced/audio/skill:2",
						...
						tinglingA: {
							...
							content: function () {
								...
								let list = [[0, "将所有花色为" + get.translation(event.choice) + "的牌交给" + get.translation(player)], [1, "跳过此阶段且非锁定技失效直到下你回合开始。"]];
						...
						yirongA: {
							...
							content: function () {
								...
								'step 3'
								if (result.bool) {
									player.reinitCharacter(player.name, result.links[0]);
								}
							},
						},
						yeyaA: {
							...
							subSkill:
							{
								mark: {
									...
									intro: {
										...
										content: function (storage, player, skill) {
											const effects = [
												`①<span style="color:#FFFF66;font-weight:bold;">无法回复体力或增加体力上限</span>`,
												`②<span style="color:#FF33CC;font-weight:bold;">无法使用或打出基本牌</span>`,
												`③<span style="color:#66FF66;font-weight:bold;">回合开始时，移除所有“蕴种印”并受到${storage}点神圣伤害</span>`
											];
											if (storage <= 0) return ""; return effects.slice(0, Math.min(storage, effects.length)).join('<br>');
										},
									},
									sub: true,
								},
								1: {
									...
									forced: true,
									trigger: {
										player: ["recoverBefore", "gainMaxHpBefore"],
									},
									filter: function (event, player) {
										return event.player.isIn() && event.player.countMark("yunzhongA_mark") >= 1;
									},
									async content(event, trigger, player) {
										trigger.cancel();
									},
									ai: {
										effect: {
											/* player函数里a、b、c、d,a、b是对你的收益，c、d是对你使用牌的目标的收益、
												 ...
												 */
											target: function (card, player, target) {
												if (card.name == "recover" || get.tag(card, "recover")) return "zeroplayertarget";
											},
										},
									},
									sub: true,
						...
						miaodiA: {
							...
							subSkill: {
								defend: {
									...
									filter: function (event, player) {
										return event.player != player && event.targets.includes(player) && get.type2(event.card) == "trick";
									},
									async content(event, trigger, player) {
										trigger.getParent().excluded.add(player);
										let gainCardList = [];
										for (var i = 0; i < 2; i++) {
											gainCardList.push(game.createCard(trigger.card));
										}
										await player.gain(gainCardList, "gain2");
									},
									ai: {
										effect: {
											target: function (card, player, target, current) {
												if (get.type2(card) == "trick" && player != target) {
													return [get.attitude(player, target), 2, 0, 0];
												}
											},
										},
									},
						...
						luanlanA: {
							...
							subSkill: {
								link: {
									...
									content: function () {
										if (player.group == "liyue") player.changeSkin({ characterName: "fengyuanwanyeA" }, "万叶1");
										else player.changeSkin({ characterName: "fengyuanwanyeA" }, "万叶2");
										game.countPlayer(function (current) {
						...
						quanmouA: {
							...
							filter: function (event, player) {
								if (!["basic", "trick"].includes(get.type(event.card))) return false;
								return get.is.damageCard(event.card) && player.isPhaseUsing();
							},
							async cost(event, trigger, player) {
								const result = await player
									.chooseTarget()
									.set("prompt", `${get.prompt(event.name.slice(0, -5))}选择一名目标角色并猜测其手牌构成`)
									.set("filterTarget", (card, player, target) => get.event().targets.includes(target))
									.set("ai", target => - get.attitude(get.player(), target) + 1)
							...
							async content(event, trigger, player) {
								...
								const result = await player.chooseButton(["权谋：猜测其有哪些类别的手牌", [list, "vcard"]], [0, 3], true)
									.set("ai", button => {
										if (!target.hasCards()) return 0;
										// 1. 获取按钮对应的类别
										const type = button.link[2].slice(8); // 去掉前缀 "caoying_"
										// 2. 获取目标手牌信息
										const known = target.getKnownCards(player);// known: 玩家可见的牌（明牌）
										const hidden = target.getCards("h", card => !known.includes(card));// hidden: 玩家不可见的牌（暗牌）
										// 3. 明牌必猜
										if (known.some(card => get.type2(card, target) === type)) {
											return 1;
										}
										// 4. 透视逻辑：对于暗牌，AI高概率猜对
										if (hidden.some(card => get.type2(card, target) === type)) {
								...
								const num = ["basic", "trick", "equip"].filter(type =>
								...
								if (num > 0) {
									const cardNameList = ["chuqibuyi", "wy_xiaolicangdao", "binglinchengxiax", "shuiyanqijunx"];
									const gainCardList = cardNameList.map(name => game.createCard(name, lib.suit.randomGet(), get.rand(1, 13)));
									await player.gain(gainCardList, "gain2");
								}
								if (num > 1) {
									target.skipList.add("phaseUse", "phaseDraw");
								}
								if (num > 2) {
									await player.addTempSkill(get.info(event.name).derivation, { player: "phaseBegin" });
								}
							},
							ai: {
								threaten: 2.4,
						...
						jinghuaA: {
							...
							ai: {
								...
								effect: {
									target: function (card, player, target) {//你成为牌的目标时对你的影响
										if (get.is.damageCard(card) && player != target) return 0.4;
										if (get.type(card) == "delay") return "zerotarget";
									},
								},
							},
							group: ["jinghuaA_skip", "jinghuaA_show"],
							subSkill: {
								skip: {
									silent: true,
									trigger: {
								...
								show:
								{
									...
									filter: function (event, player) {
										return event.player != player && event.player.hasCards();
									},
									async content(event, trigger, player) {
						...
						fenchuanA: {
							...
							filter: function (event, player) {
								return event.player.isIn() && event.player != player && event.getParent().name != 'fenchuanA';
							},
							async content(event, trigger, player) {
								const damageNumber = player.getAllHistory("useCard", evt => get.is.damageCard(evt.card)).length + 1;
								const choice = ["万水一露", "滥觞无底"];
								const choiceList = [
									`万水一露：对${get.translation(trigger.player)}造成${get.cnNumber(damageNumber)}次1点伤害`,
									`滥觞无底：令${get.translation(trigger.player)}进行一次判定，若判定结果不大于${damageNumber}，其立即死亡`];
								const result = await player.chooseControl(choice)
									.set("displayIndex", false)
									.set("prompt", `${get.translation(player)}发动了【分川】，请选择一项：`)
									.set("choiceList", choiceList)
									.set("ai", function () {
										if (damageNumber >= 6 || get.damageEffect(trigger.player, player, player) < 0) return 1;
										else return 0;
									})
									.forResult();
								if (result.index === 0) {
									for (let index = 0; index < damageNumber; index++) {
										await trigger.player.damage();
									}
								}
								else {
									const judgeResult = await trigger.player.judge().forResult();
									if (judgeResult.number <= damageNumber) {
										await trigger.player.die({ source: player });
									}
								}
							},
						...
						bingceA: {
							...
							enable: "chooseToUse",
							filter(event, player) {
								if (!player.hasCard(card => get.type2(card) != "trick", "hes")) return false;
								return get.inpileVCardList(info => {
									if (info[0] != "trick") return false;
									if (player.getStorage("bingceA_used").includes(info[2])) return false;
									return event.filterCard(get.autoViewAs({ name: info[2], storage: { bingceA: true } }, 'unsure'), player, event)
								}).length
							},
							chooseButton: {
								dialog(event, player) {
									...
									const dialog = ui.create.dialog('请选择一张锦囊牌', [list, 'vcard']);
									return dialog;
								},
								filter: function (button, player) {
									return _status.event.getParent().filterCard({ name: button.link[2], storage: { bingceA: true } }, player, _status.event.getParent());
								},
								check: function (button) {
									if (_status.event.getParent().type != 'phase') return 1;
									const player = get.player();
									if (['wugu', 'zhulu_card', 'yiyi', 'lulitongxin', 'lianjunshengyan', 'diaohulishan', 'diaobingqianjiang'].includes(button.link[2])) return 0;
									return player.getUseValue({ name: button.link[2], storage: { bingceA: true } });
								},
								backup: function (links, player) {
									let next = {
										audio: "bingceA",
										popname: true,//控制视为使用牌的是否弹出牌名特效
										selectCard: 1,
										filterCard: (card) => get.type2(card) != "trick",
										position: 'hes',
										viewAs: { name: links[0][2], storage: { bingceA: true } },
										async precontent(event, trigger, player) {
											player.addTempSkill("bingceA_used");
											player.markAuto("bingceA_used", [event.result.card.name]);
											player
								...
								prompt(links) {
									return `将一张非锦囊牌当做${get.translation(links[0][2])}使用`;
								},
							},
							hiddenCard: function (player, name) {
								if (!lib.inpile.includes(name)) return false;
								if (player.getStorage("bingceA_used").includes(name)) return false;
								return get.type(name) == "trick" && player.hasCard(card => get.type2(card) != "trick", "hes");
							},
							subSkill: {
								used: {
									charlotte: true,
							...
							mod: {
								targetInRange: function (card, player, target, now) {
									if (get.type(card) == 'trick' && card.storage && card.storage.bingceA) return true;
								},
							},
							ai: {
								order: 8,
						...
						tengzhiA: {
							...
							filter: function (event, player) {
								return event.player.isIn() && game.hasPlayer((current) => current != player && !current.hasMark("tengzhiA_1"));
							},
							async content(event, trigger, player) {
								const result = await player.chooseTarget([1, Infinity])
									.set("filterTarget", (card, player, target) => target != player && !target.hasMark("tengzhiA_1"))
									.set("prompt", get.prompt2("tengzhiA"))
									.set("ai", (target) => get.attitude(player, target)).forResult();
								if (result.bool) {
									result.targets.sortBySeat();
									for (let target of result.targets) {
										target.addMark("tengzhiA_1", 1);
									}
								}
							},
							group: ["tengzhiA_1", "tengzhiA_2", "tengzhiA_3"],
							subSkill: {
								1: {
									audio: "ext:原梗Enhanced/audio/skill:2",
									charlotte: true,
									...
									filter: function (event, player) {
										...
										return event.player.isIn() && event.player.hasMark("tengzhiA_1") && event.num >= event.player.hp;
									},
									async content(event, trigger, player) {
										trigger.player.removeMark("tengzhiA_1", 1);
										trigger.cancel();
										await player.damage(trigger.source ? trigger.source : 'nosource', trigger.nature, trigger.num).set('card', trigger.card).set('cards', trigger.cards);
									},
									sub: true,
								},
								2: {
								...
								3: {
									...
									ai: {
										effect: {
											target: function (card, player, target, current) {
												if (!get.tag(card, "fireDamage") && player != target) {
						...
						jianwuA: {
							audio: "ext:原梗Enhanced/audio/skill:2",
							init: (player, skill) => player.changeHujia(1),
							forced: true,
							trigger: {
								source: "damageSource",
							},
							...
							filter: function (event, player) {
								return event.player != player && event.num && player.hujia < player.maxHp;
							},
							async content(event, trigger, player) {
								await player.changeHujia(1, "gain", player.maxHp);
							},
							group: "jianwuA_1",
							subSkill: {
								1: {
									...
									filter: function (event, player) {
										if (get.tag(event.card, "norepeat") || get.type(event.card) == "delay") return false;
										return event.player.isIn() && get.tag(event.card, "damage");
									},
									async content(event, trigger, player) {
							...
							mod: {
								targetEnabled: function (card, player, target) {
									if (player != target && !player.inRange(target) && get.type(card) == "trick") return false;
								},
							},
						},
						yinyanA: {
							...
							forced: true,
							filter: function (event, player) {
								return (event.name != "phase" || game.phaseNumber == 0) && player.hasEquipableSlot(5) && !player.getEquips("jianyuehuanA").length;
							},
							async content(event, trigger, player) {
								const card = game.createCard2("jianyuehuanA", "heart", 6);
								player.$gain2(card, false);
								await game.delayx();
								await player.equip(card);
							},
							group: "yinyanA_blocker",
							subSkill: {
								blocker: {
									trigger: {
										player: ["loseBefore", "disableEquipBefore"],
									...
									filter: function (event, player) {
										...
										let cards = player.getEquips("jianyuehuanA");
										return event.cards.some(card => cards.includes(card));
									},
									async content(event, trigger, player) {
										if (trigger.name == "lose") {
											trigger.cards.removeArray(player.getEquips("jianyuehuanA"));
										} else {
							...
							mod: {
								canBeGained(card, source, player) {
									if (player.getEquips("jianyuehuanA").includes(card)) return false;
								},
								canBeDiscarded(card, source, player) {
									if (player.getEquips("jianyuehuanA").includes(card)) return false;
								},
								canBeReplaced(card, player) {
									if (player.getVEquips("jianyuehuanA").includes(card)) return false;
								},
								cardname(card, player) {
									if (get.subtype(card, false) == "equip5") return "dongzhuxianji";
								},
								cardDiscardable(card, player) {
									if (player.getEquips("jianyuehuanA").includes(card)) return false;
								},
					...
					translate: {//各种技能的翻译，不写等着全是英文吧
						...
						HunYinA_2: "元素",
						"HunYinA_2_info": '锁定技。①你防止受到的非属性伤害。②当其他角色造成属性伤害后，你获得其所有牌。',
						HunYinA_3: "能量",
						"HunYinA_3_info": '锁定技。当你受到伤害后，可以令任意名其他角色受到两倍无来源的神圣伤害并翻面。',
						HunYinA_4: "生命",
						"HunYinA_4_info": '锁定技。你的回合外，你的体力与体力上限不会减少。',
						HunYinA_5: "轮回",
						"HunYinA_5_info": '锁定技。每回合限七次，你使用牌后重新获得此牌对应的实体牌。',
						HunYinA_6: "永恒",
						"HunYinA_6_info": '锁定技。其他角色的回合结束后，你获得一个额外的回合。',
						HunYinA_7: "圣洁",
						"HunYinA_7_info": '锁定技。当你获得此技能时，你获得游戏的胜利。',
						seer_leiji: "雷祭",
						...
						seer_zhanhun: "战魂",
						seer_zhanhun_info: "异能技。①当你于出牌阶段使用或打出一张牌后，若你未记录此牌花色，你记录此牌花色并获得未拥有的伤害类牌各一张。②回合开始时，你清除〖战魂〗①记录的花色并弃置判定区内的所有牌。③你于出牌阶段内使用牌无次数距离限制。",
						seer_jingtian: "惊天",
						seer_jingtian_info: "限定技。当你使用【杀】指定其他角色为唯一目标时，若〖战魂〗①记录的花色数不小于4，你令此牌不可被响应且伤害+目标角色体力上限，然后目标角色失去武将牌上的所有非异能技。",
						seer_kuangwu: "狂武",
						seer_kuangwu_info: "①每回合限三次，当你受到其他角色造成的伤害时，若你本回合未对其造成过伤害，防止此伤害。②回合结束时，若你本回合内杀死过角色，你令〖惊天〗视为未发动过。",
						ranhuaA: "燃花",
						...
						fengshenA: "封神",
						"fengshenA_info": "出牌阶段限一次，你可以弃置等同于“磐石”数的基本牌和全部“磐石”，选择任意名其他角色，对每名角色造成X点岩属性伤害。（X为磐石标记数和3之间的较大值)",
						wuxiangA: "无想",
						...
						"yanshouC_info": "",
						jichouA: "记仇",
						jichouA_info: "锁定技。①当你受到其他角色造成的伤害后，你须将其所有牌置于你的武将牌上称为“仇”，然后其失去X点体力（X为伤害值）。②回合开始时，你弃置所有的“仇”并并视为使用至多等量张无次数距离限制的冰【杀】",
						zhuomengA: "酌梦",
						zhuomengA_info: `①其他角色的回合结束时，若你在其攻击范围内且你本回合未受到伤害,你视为使用一张【酒】。②当你使用【酒】后，你获得${get.ygeBolInformX("幻梦牌", configData.customText.huanMengCard)}各一张。`,
						jiawuA: "枷舞",
						jiawuA_info: "使命技。①游戏开始时，你获得三枚“枷锁”标记。你无法使用或打出【闪】/【桃】。②当你对其他角色造成伤害后，你移除一枚“枷锁”。③成功：当你使用一张牌后，若你无“枷锁”，你获得“浪舞”。④失败：当你进入濒死状态时，你回复体力至1点。",
						langwuA: "浪舞",
						langwuA_info: "锁定技。①每回合每种牌名限一次，当你使用一张牌时，你从牌堆获得三张普通锦囊牌。②结束阶段开始时，你选择一名其他角色并对其造成X点冰冻伤害。（X为你本回合使用的牌数）",
						huolinA: "获麟",
						...
						yanmingA: "延命",
						"yanmingA_info": "每回合限三次,一名角色进入濒死状态时，你可以令该角色进行一次判定，若判定结果不为♠2~♠9，则其将体力值回复至1点并摸等同于其体力上限数张牌。",
						huihaiA: "回骸",
						"huihaiA_info": "出牌阶段限一次，你可以减一点体力上限并选择一名已死亡的其他角色，令该角色复活并将体力和体力上限调整为X，然后摸X张牌（X为目标角色武将牌上的体力上限）。",
						huohuaA: "火花",
						huohuaA_info: "锁定技。当你使用或打出基本牌结算后，若此牌点数为：质数，你可令任意名其他角色各交给你一张与此牌颜色相同的牌，否则受到1点无来源的火焰伤害。然后你获得2枚“火花”标记；非质数，你可以视为使用一张无次数距离限制且可以指定任意名目标的火【杀】。此牌对目标角色造成伤害后，若其手牌中伤害类牌多于/不多于非伤害类牌，其获得〖焚烬〗/〖止息〗至其下回合结束。",
						baodanA: "爆弹",
						baodanA_info: "出牌阶段限一次，你可以移除所有“火花”并对一名体力值不高于“火花”数其他角色造成X点火焰伤害（X为你的“火花”标记数），若目标角色未死亡，则其随机失去武将牌上的一个技能。",
						tianjuanA: "天眷",
						tianjuanA_info: "①锁定技，每回合限三次，当你受到伤害时，你可以摸两张牌，若其中包含基本牌则防止此伤害。②判定阶段开始时，你可以跳过此阶段并从牌堆或弃牌堆中获得点数为质数的牌各一张。",
						fenjinA: "焚烬",
						fenjinA_info: "当你使用或打出基本牌或普通锦囊牌结算后，若此牌为红色，你对自己造成X点火焰伤害(X为你已损失的体力值且至少为1)，反之，则将此牌置入你的武将牌上称为“烬”。你无法使用或打出与“烬”同牌名的牌。",
						yanxiaoA: "焰硝",
						...
						shanyaoA_tag: "星",
						shanyaoA: "闪耀",
						shanyaoA_info: "锁定技。①游戏开始时，你将初始手牌标记为“星”且你的初始手牌数为十。②你的“星”不计入手牌上限。③当你失去一张类型为基本牌的“星”后，你摸两张牌并标记为“星”。",
						qihuA: "祈护",
						qihuA_info: "一名角色的回合开始时，你可以弃置至多三张“星”，若你弃置的“星”数至少为：一，其弃置判定区内的所有牌；二，其本回合使用基本牌无次数距离限制；三，其本回合使用基本牌的牌面数值+1。",
						lianyinA: "涟音",
						lianyinA_info: "出牌阶段限一次，你可以弃置至多三张“星”并令等量名其他角色回复体力至上限，然后这些角色依次选择一项：1.令你获得其手牌中所有的【杀】并标记为“星”；2.翻面。",
						chunxinA: "纯心",
						chunxinA_info: "锁定技。①当你受到非基本牌造成的伤害时，防止此伤害，然后你获得基本牌各一张并标记为“星”。②一名角色处于濒死状态时，你可以将一张“星”当做【桃】使用。",
						huobanA: "伙伴",
						...
						yizhouA: "役咒",
						"yizhouA_info": "锁定技。①游戏开始时,你将牌堆顶的七张牌扣置于武将牌上称为“樱”。②每回合限五次，当你使用伤害类牌指定其他角色为目标时，若“樱”数小于20，你可以将目标角色的至多三张牌扣置于你的武将牌上称为“樱”，若你未扣置牌且体力上限小于10，你增加1点体力上限并回复1点体力。③每回合限三次，当你成为其他角色伤害类牌的目标时，其需弃置2X张牌，否则此牌对你无效且你获得此牌对应的实体牌（X为“樱”包含的花色数）。",
						yizhouB: "役咒",
						"yizhouB_info": "每回合限三次，当你成为其他角色伤害类牌的目标时，其需弃置2X张牌，否则此牌对你无效且你获得此牌对应的实体牌（X为“樱”包含的花色数）。",
						xianzhenA: "显真",
						"xianzhenA_info": "每回合限Y次,你可以将一张“樱”当作无距离和次数限制的任意基本牌使用或打出，然后你令一名其他角色进行一次判定,若判定结果为：红色，其失去1点体力；黑色，其受到1点无来源的雷电伤害。（Y为你的体力上限）。",
						shenleA: "神乐",
						shenleA_info: "①出牌阶段限一次，若你的体力上限大于9，你可以失去9点体力上限并令任意名其他角色随机执行一项：1.失去等量体力上限；2.受到等量次1点雷电伤害。②一名角色因〖神乐〗①死亡后，你增加1点体力上限并获得三张普通锦囊牌。",
						xuanjiA: "璇玑",
						...
						miaosuanA: "庙算",
						"miaosuanA_info": "①锁定技，摸牌阶段开始时，你跳过之，改为依次选择指定获得某种类型的牌（重复X次，X为场上角色数和你体力值间的较大值），然后从牌堆随机摸取之；②锁定技，你使用普通锦囊牌不可被响应。当你使用一张锦囊牌后，你有概率发现四张随机锦囊牌，然后选择其中一至两张获得(初始概率为100%，你每发动一次技能便减少15%，最低为10%，每回合重置)。",
						hairanA: "海染",
						...
						nuomianA: "傩面",
						nuomianA_info: "①每回合限两次，当你成为其他角色使用牌的目标时，你可以观看其手牌并选择其中一张手牌标记为“裂解”牌。②锁定技，有“裂解”牌的角色不能使用或打出与“裂解”牌同类型的牌，且当其失去“裂解”牌时，其失去1点体力上限。③锁定技，手牌数不小于你的其他角色无法使用【杀】指定你为目标。",
						nuomianA_tag: "裂解",
						yezhangA: "业障",
						yezhangA_info: "出牌阶段开始时，你可以令自己的所有手牌于此阶段内均视为" + get.ygeBolInformX("风【杀】", configData.customText.fengshaA) + "。若如此做，你获得下列效果至回合结束：1.使用【杀】无次数和距离限制且可以指定任意名目标；2.使用【杀】无视防具且基础伤害+1；3.当你使用【杀】对其他角色造成伤害后，若其因此死亡，你失去1点体力。",
						yezhangA_buff: "修罗",
						yezhangA_debuff: "噬心",
						yezhangA_debuff_info: "当你使用【杀】对其他角色造成伤害后，若其因此死亡，你失去1点体力。",
						yezhangA_buff_info: "锁定技。①你使用【杀】无次数和距离限制且可以指定任意名目标。②你使用【杀】无视防具且基础伤害+1。",
						xiangmoA: "降魔",
						xiangmoA_info: "觉醒技，一名角色的回合开始时，若你的手牌数或体力值小于等于2，你回复体力至上限,然后失去〖业障〗并获得〖靖妖〗、〖连破〗、〖完杀〗（界）。",
						jingyaoA: "靖妖",
						jingyaoA_info: "锁定技。①你使用【杀】无次数和距离限制且可以指定任意名目标。②你使用【杀】无视防具、不可响应且基础伤害+2。③每回合限三次，你可以将一张牌当作" + get.ygeBolInformX("风【杀】", configData.customText.fengshaA) + "使用或打出。",
						cixiongA: "雌雄",
						...
						langgeA: "朗歌",
						langgeA_info: "每回合限三次，当你使用或打出一张伤害类牌后，若此牌与你上一张使用的牌的花色/牌名/点数不同，你可以选择一项：1.获得一张【无中生有】和一张【推心置腹】；2.获得一张【酒】和一张【兄弟齐心】；3.获得一张【杀】和一张【决斗】。",
						leilunA: "雷轮",
						...
						lianjinA: "炼金",
						"lianjinA_info": "锁定技，回合开始时，你投掷一枚骰子。若X>=2，你随机获得X张" + get.ygeBolInformX("药剂", configData.customText.yaoJiCard) + "（X为骰子点数）,否则你获得1张【毒】。",
						lianjin_mabi: "麻痹",
						lianjin_zhuoshao: "灼烧",
						fenglingA: "风灵",
						"fenglingA_info": "出牌阶段限一次，你可以弃置任意张牌并令等量名其他角色各受到1点风属性伤害。",
						nizaoA: "拟造",
						...
						baiweiA: "百味",
						"baiweiA_info": "①当你对一名角色造成火焰伤害时，你获得2X个“焰火”标记（X为伤害值）。②出牌阶段限一次，你可以移除全部“焰火”标记并获得等量张" + get.ygeBolInformX("佳肴牌", configData.customText.jiaYaoCard) + "③你的佳肴牌不计入手牌上限",
						baiweiB: "百味",
						zaoshenA: "灶神",
						...
						tinglingA: "听令",
						"tinglingA_info": "其他角色的出牌阶段开始前，你可以指定1种花色并令其选择一项：1.交给你指定花色的所有牌，2.跳过此阶段且非锁定技失效至其下回合开始。",
						yirongA: "易容",
						...
						yunzhongA: "蕴种",
						yunzhongA_info: "①出牌阶段限一次，你可以令任意名其他角色各获得1枚“蕴种印”标记。②拥有“蕴种印”的角色依“蕴种印”标记数量获得如下效果：≥1，无法回复体力或增加体力上限；≥2，无法使用或打出基本牌；≥3，回合开始时，移除所有“蕴种印”并受到X点神圣伤害（X为“蕴种印”数量）。",
						miaodiA: "妙谛",
						miaodiA_info: "①锁定技，一名其他角色的回合结束后，若其本回合使用了>=Y张锦囊牌，你获得一个额外的回合（Y为其当前体力值）。②锁定技，当其他角色使用锦囊牌指定你为目标时，此牌对你无效且你获得同牌名的两张锦囊牌。",
						fumengA: "浮梦",
						...
						chirenA: "赤刃",
						chirenA_info: "璃月技，锁定技，①判定/弃牌阶段开始时，你可以跳过此阶段，视为使用一张无次数、距离限制，且可以指定任意名目标的" + get.ygeBolInformX("风【杀】", configData.customText.fengshaA) + "。②一名其他角色的回合结束时，你获得本回合进入弃牌堆的非伤害类牌。",
						kongjingA: "空净",
						kongjingA_info: "稻妻技，①每回合限五次，当其他角色使用普通锦囊牌或带有「伤害」标签的基本牌指定你为目标时，其需交给你一张类型、花色均相同的牌，否则你令此牌无效并获得其一张牌。②锁定技，其他角色无法使用延时锦囊牌指定你为目标。③锁定技，一名角色的回合结束时，若你本回合未受到过伤害，则你将手牌摸至X张。（X为全场已横置角色数和你的体力上限间的较大值）",
						exiA: "恶戏",
						...
						quanmouA: "权谋",
						quanmouA_info: "每回合限一次，当你于出牌阶段使用伤害类基本牌或普通锦囊牌指定其他角色为目标后，你可以猜测其中的一个目标的手牌中是否有基本牌，锦囊牌或装备牌。若你至少猜对了：一项，你获得【笑里藏刀】、【出其不意】、【兵临城下】、【水淹七军】各一张；两项，你令其跳过下个摸牌阶段和出牌阶段；三项，你获得〖秉策〗至下回合开始。",
						jinghuaA: "镜花",
						jinghuaA_info: "锁定技。①每回合限三次，当其他角色使用伤害类基本牌或普通锦囊牌指定你为目标时，你令此牌对你无效并获得此牌对应的实体牌。②其他角色的回合结束时，你令其展示所有手牌。③你始终跳过判定阶段。",
						fenchuanA: "分川",
						fenchuanA_info: "锁定技。当你不因此技能对其他角色造成伤害后，你可以选择一项：1.对其造成X次1点伤害；2.令其进行一次判定，若判定结果点数不大于X，其立即死亡。（X为本局游戏内你使用的伤害类牌数+1）",
						bingceA: "秉策",
						bingceA_info: "你可以将一张非锦囊牌当做本回合未以此法使用过的无距离限制的任意普通锦囊牌使用，然后你摸两张牌。",
						...
						jianwuA: "翦舞",
						jianwuA_info: "锁定技。①你初始拥有1点护甲。当你对其他角色造成1点伤害后，你获得1点护甲(不超过你体力上限)。②每回合限三次，当你使用伤害类牌时，此牌额外结算两次。③你攻击范围外的其他角色无法使用普通锦囊牌指定你为目标。",
						tengzhiA_1: "藤护",
						tengzhiA_1_info: "拥有“藤护”的其他角色受到致命伤害时，若你不为伤害来源，则其移除一枚“藤护”并将此伤害转移给你。",
						tengzhiA_2: "火焚",
				...
				for (var i in genshinImact.character) {
					...
					genshinImact.character[i][4].push(`skinPath:ext:原梗Enhanced/skin/image/${i}/`);
				}
				return genshinImact;
			});
			let charPack = {//添加武将包
			...
			game.import('card', function () {
				var GenshinImpactEnhanced = {
					...
					card: {//各种DIY卡牌的放置区域，需要注意的是，卡牌调用的技能不要写在这里，而是当做普通技能写在之前的地方。
						...
						jianyuehuanA: {
							fullskin: true,
							image: "ext:原梗Enhanced/image/card/翦月环.png",
							type: "equip",
							subtype: "equip5",
							skills: ["jianyuehuanA_skill_1", "jianyuehuanA_skill_2"],
							ai: {
								basic: {
									equipValue: 3,
									order: (card, player) => {
					...
					skill: {
						...
						jilijianA_skill: {
							...
							filter: function (event, player) {
								if (!["trick", "basic"].includes(get.type(event.card))) return false;
								if (get.tag(event.card, "norepeat") || get.info(event.card).notarget) return false;
							...
							check: function (event, player) {
								return !get.tag(event.card, "norepeat") && !get.info(event.card).notarget;
							},
							async content(event, trigger, player) {
								trigger.effectCount++;
						...
						jianyuehuanA_skill_1: {
							equipSkill: true,
							forced: true,
							trigger: {
								player: ["loseHpBegin", "loseMaxHpBegin", "phaseJudgeBefore"]
							},
							filter: function (event, player) {
								return event.player.isIn();
							},
					...
					translate: {//卡牌的翻译，不写的话游戏里可能无法加载出来
						...
						tianquanbengyuA_info: "出牌阶段，对所有其他角色使用。每名目标角色需要打出两张【闪】，否则受到1点岩属性伤害。",
						lieyanyaoshuiA: "烈焰药水",
						...
						amosizhigongA_skill: "阿莫斯之弓",
						jianyuehuanA: "翦月环",
						jianyuehuanA_info: "锁定技。①当你对其他角色造成伤害时，此伤害追加风属性。②你不会失去体力和体力上限。③你始终跳过判定阶段。",
						jianyuehuanA_append: `<div style="width:100%;text-align:left;font-size:13px;font-style:italic">燕子来时挟清雨，能洗旧冬一点愁。</div>`,
						delicacies: "佳肴",
						xiantiaoqiangA: "仙跳墙",
